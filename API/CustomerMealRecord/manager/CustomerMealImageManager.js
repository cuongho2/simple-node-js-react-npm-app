...
function _checkCustomerRecordDate(customerRecordData, autoFill = true) {
	...
	if (customerRecordData.customerRecordCheckExpiredDate === undefined) {
		if (autoFill) {
			customerRecordData.customerRecordCheckExpiredDate = moment()
				.add(customerRecordData.customerRecordCheckDuration, "M")
				.toDate();
		}
	} else {
		let checkExpiredDate = moment(
...
async function find(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let customerRecord = await CustomerRecordResourceAccess.customSearchByExpiredDate(
				filter,
				skip,
				limit,
				startDate,
			...
			let customerRecordCount = await CustomerRecordResourceAccess.customCountByExpiredDate(
				filter,
				startDate,
				endDate,
				searchText,
...
async function findToday(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let customerRecord = await CustomerRecordResourceAccess.customSearch(
				filter,
				skip,
				limit,
				startDate,
			...
			let customerRecordCount = await CustomerRecordResourceAccess.customCount(
				filter,
				startDate,
				endDate,
				searchText,
...
async function updateById(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let result = await CustomerFuntion.updateCustomerRecordById(
				customerRecordId,
				customerRecordData,
			);
			if (result) {
		...
		try {
			...
			if (newExcel) {
				...
				if (excelData === undefined) {
					...
					let needToImportRecords =
						await CustomerFuntion.convertExcelDataToCustomerRecord(
							excelData,
							stationsId,
						);
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			fs.readFile(payload.image.path, (err, data) => {
				...
				let newFileName = `station_${station.stationsId}_at_${moment().format(
					"YYYYMMDDhhmmss",
				)}${path.extname(payload.image.filename)}`;
				...
				fs.writeFile(`${dirName}/${newFileName}`, data, async (writeErr) => {
					...
					let newCustomerRecordData = {
						...
						customerRecordPlateImageUrl: `${process.env.HOST_NAME}/${dirName}/${newFileName}`,
