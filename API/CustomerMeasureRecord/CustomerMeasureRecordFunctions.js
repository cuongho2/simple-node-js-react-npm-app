...
_fetchYoujiToken().then((token) => {
  console.log(token)
  _fetchYoujiReportListByPhone("0943529669").then((result) => {
...
async function _fetchYoujiDetailForAllRecord(recordList) {
  ...
  for (let i = 0; i < recordList.length; i++) {
    ...
    promiseList.push(fetchRecordDetailById(_record.id));
    
  }
  let measureResult = await Promise.all(promiseList);
  for (let i = 0; i < measureResult.length; i++) {
    const measureRecord = measureResult[i];
