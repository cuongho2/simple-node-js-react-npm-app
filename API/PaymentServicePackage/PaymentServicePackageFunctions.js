...
const DEFAULT_PACKAGE_DURATION = 361; //mac dinh la 361 ngay
async function createPackagesByPackageType(packageType, packageCategory = PACKAGE_CATEGORY.NORMAL) {
...
async function generatePresalePackages(packageType, maxNumberOfPackages) {
  ...
  for (let i = 0; i < needToGenerate; i++) {
    await createPackagesByPackageType(packageType);
  }
}
...
async function addBonusPackageForUser(appUserId, packageType) {
  ...
  let _newPackageId = await createPackagesByPackageType(packageType, PACKAGE_CATEGORY.BONUS);
  if (!_newPackageId) {
