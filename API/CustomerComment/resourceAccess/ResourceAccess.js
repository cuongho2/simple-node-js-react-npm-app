...
const tableName = "CustomerComment";
...
async function seeding() {
	return new Promise(async (resolve, reject) => {
		let initialBrands = [
			{
				content:
					"Mình đã mua ở rất nhiều nơi rồi, Lofi Style có thể là điểm đến cuối cùng của mình, bán toàn đồ chất lượng.",
				title: "",
				productId: "4",
				userId: "4",
			},
			{
				content:
					"Không phải tự dưng mà tuy khen shop này đâu nhé. Đồ gì đâu mà vừa chất lượng lại vừa có giá vô cùng rẻ nữa chứ.",
				title: "",
				productId: "2",
				userId: "2",
			},
...
function _makeQueryBuilderByFilter(filter, skip, limit, order) {
	...
	queryBuilder.join("AppUser", "AppUser.appUserId", "=", `${tableName}.userId`);
	queryBuilder.options({ nestTables: true });
