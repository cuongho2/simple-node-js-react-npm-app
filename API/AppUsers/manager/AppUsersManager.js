...
async function _registerUser(userData) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (insertResult) {
				if (userData.userServiceId) {
					const _userServiceId = userData.userServiceId;
					const AppUserServiceFunctions = require("../../AppUserService/AppUserServiceFunctions");
					await AppUserServiceFunctions.storeUserService(insertResult[0], _userServiceId);
				}
...
async function find(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let usersCount = await AppUsersResourceAccess.customCount(
				filter,
				searchText,
				startDate,
				endDate,
...
async function _existedUserEmail(email) {
	if (email) {
...
async function _existedUserPhoneNumber(phoneNumber) {
	if (phoneNumber) {
		let user = await AppUsersResourceAccess.find({ phoneNumber: phoneNumber });
		if (user && user.length > 0) {
			return user[0];
		}
	}
...
async function updateById(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (await _existedUserPhoneNumber(userData.phoneNumber)) {
				reject(USER_ERROR.DUPLICATED_USER_PHONE);
				return; //always add "return" after reject / resolve to make sure everything will break
			} else if (await _existedUserEmail(userData.email)) {
				reject(USER_ERROR.DUPLICATED_USER_EMAIL);
				return; //always add "return" after reject / resolve to make sure everything will break
			}
...
async function userDeleteAccount(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let updateResult = await AppUsersResourceAccess.updateById(appUserId, {
				isDeleted: true,
				username:
					"deleted_" + moment().format("YYYYMMDDHHmmSS") + "_" + req.currentUser.username,
				email:
...
async function findById(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (foundUser) {
				if (foundUser.userServiceId) {
					const AppUserServiceFunctions = require("../../AppUserService/AppUserServiceFunctions");
					let _userServices = await AppUserServiceFunctions.fetchAllStationServicesByUser(
						insertResult[0],
					);
					foundUser.userServices = _userServices;
				}
		...
		try {
			...
			if (users && users.length > 0) {
				...
				if (foundUser) {
					...
					let verified = AppUsersFunctions.verify2FACode(
						otpCode.toString(),
						foundUser.twoFACode,
					);
...
async function loginFacebook(req) {
	return new Promise(async (resolve, reject) => {
		try {
			if (
				req.payload.facebook_id &&
				req.payload.facebook_id !== "" &&
				req.payload.facebook_id !== null
			) {
				...
				let foundUser = await _loginSocial(
					username,
					password,
					firstName,
					email,
...
async function loginApple(req) {
	return new Promise(async (resolve, reject) => {
		try {
			if (
				req.payload.apple_id &&
				req.payload.apple_id !== "" &&
				req.payload.apple_id !== null
			) {
...
async function userUpdateInfo(req) {
	return new Promise(async (resolve, reject) => {
		try {
			let userData = req.payload.data;
			let id = req.currentUser.appUserId;
			let _existedUser = undefined;
			if (userData.phoneNumber) {
				_existedUser = await _existedUserPhoneNumber(userData.phoneNumber);
				if (_existedUser && _existedUser.username !== req.currentUser.username) {
					console.log(USER_ERROR.DUPLICATED_USER_PHONE);
					reject(USER_ERROR.DUPLICATED_USER_PHONE);
					return; //always add "return" after reject / resolve to make sure everything will break
				}
			}
			if (userData.email) {
				_existedUser = await _existedUserPhoneNumber(userData.email);
				if (_existedUser && _existedUser.username !== req.currentUser.username) {
					console.log(USER_ERROR.DUPLICATED_USER_EMAIL);
					reject(USER_ERROR.DUPLICATED_USER_EMAIL);
					return; //always add "return" after reject / resolve to make sure everything will break
				}
			}
			...
			if (updateResult !== undefined) {
				const foundUser = await AppUsersFunctions.retrieveUserDetail(id);
				if (foundUser) {
					resolve(foundUser);
				} else {
			...
			for (let i = 0; i < users.length - 1; i++) {
				for (let j = i + 1; j < users.length; j++) {
					if (users[i].createYear > users[j].createYear) {
						...
						users[i].createMonth > users[j].createMonth &&
		...
		try {
			...
			if (user === undefined || user.appUserId === null) {
				...
				if (result) {
					...
					if (process.env.GOOGLE_FIREBASE_PUSH_ENABLE) {
						handleSendMessageUser(APPROVE_USER_INFO, messageData, user.appUserId, {
							validated: true,
						});
					}
...
async function adminChangeSecondaryPasswordUser(req) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (foundUser && foundUser.length > 0) {
				let result = await AppUsersFunctions.changeUserSecondaryPassword(
					foundUser[0],
					newPassword,
				);
				if (result) {
