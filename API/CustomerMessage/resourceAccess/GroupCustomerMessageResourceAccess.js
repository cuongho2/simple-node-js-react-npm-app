...
function _makeQueryBuilderByFilter(filter, skip, limit, startDate, endDate, searchText, order) {
  ...
  if (searchText) {
    queryBuilder.where(function() {
      this.orWhere('groupCustomerMessageContent', 'like', `%${searchText}%`)
      .orWhere('groupCustomerMessageTitle', 'like', `%${searchText}%`)
    })
  } else {
    if(filterData.groupCustomerMessageContent){
      queryBuilder.where('groupCustomerMessageContent', 'like', `%${filterData.groupCustomerMessageContent}%`)
      delete filterData.groupCustomerMessageContent;
    }

    if(filterData.groupCustomerMessageTitle){
      queryBuilder.where('groupCustomerMessageTitle', 'like', `%${filterData.groupCustomerMessageTitle}%`)
      delete filterData.groupCustomerMessageTitle;
    }
  }
